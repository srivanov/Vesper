...
void World_BlackBoard::addRecord(TypeRecords& type, int * id, vector3D * info){
    Key * _key = new Key(type,id);
    record * _record = new record(info);
    _map.insert(std::pair<Key* , record *> (_key,_record));
}
void World_BlackBoard::addRecord(TypeRecords& type, int* id, int* idTarget, vector3D* info){
...
bool World_BlackBoard::regulator(vector3D* info) const{
...
Personal_BlackBoard::Personal_BlackBoard(unsigned int& life, unsigned int& hungry, unsigned int& thirst, vector3D* position) :
...
void Personal_BlackBoard::update(unsigned int& life, unsigned int& hungry, unsigned int& thirst, vector3D* position){
    _life = life; _hungry = hungry; _thirst = thirst; _position = position;
}
...
void NPC_library::AddBook(int* id, unsigned int& life, unsigned int& hungry, unsigned int& thirst, vector3D* position){
    it = _values.find(id);
    if(it!=_values.end())
        return;
...
void NPC_library::updateMyBook(int* id, unsigned int& life, unsigned int& hungry, unsigned int& thirst, vector3D* position){
    it = _values.find(id);
    if(it==_values.end())
        return;
