...
PathPlanning * PathPlanning::pinstance = 0;
...
void PathPlanning::init(Graph * grafo){
    pinstance = new PathPlanning(grafo);
}
...
std::vector<dvector3D*> PathPlanning::obtenerCamino(dvector3D *initialPosition,dvector3D * finalPosition){
    ...
    // AÑADE NODO INICIAL
    BolsaNodos->add_node(grafo->getInitialNode(initialPosition));
...
bool PathPlanning::EvaluarCamino(){
    ...
    BolsaNodosAux.add_node(grafo->getInitialNode(aux));
