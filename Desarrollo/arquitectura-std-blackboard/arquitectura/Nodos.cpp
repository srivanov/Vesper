...
short NodoMover::run(int &id){
    ...
    if(aux==-1){
        ...
        if(fabsf(xABS)<fabsf(yABS) && (fabsf(xABS)-fabsf(yABS))<0.05){
            float y = 2.5;
            if (yABS<0) y=-2.5;
            _movement->x = 0;
            _movement->y = y;
        ...
        else if(fabsf(xABS)>fabsf(yABS) && (fabsf(xABS)-fabsf(yABS))<0.05){
            float x = 2.5;
            if (xABS<0) x=-2.5;
            _movement->x = x;
            _movement->y = 0;
...
short NodoVigilar::run(int &id){
    ...
    if(NPC_library::instance()->getMyBook(&id)->getPosObjetivo()!=vigilar)
        ...
        if(aux==-1){
            ...
            xABS = NPC_library::instance()->getMyBook(&id)->getPosObjetivo()->x-NPC_library::instance()->getMyBook(&id)->getPosition()->x;
            yABS = NPC_library::instance()->getMyBook(&id)->getPosObjetivo()->y-NPC_library::instance()->getMyBook(&id)->getPosition()->y;
        }else if(aux>0 && aux<1.1){
            aux=-1;
            return true;
        }else{
            if(fabsf(xABS)<fabsf(yABS) && (fabsf(xABS)-fabsf(yABS))<0.5){
                float y = 2.5;
                if (yABS<0) y=-2.5;
                _movement->x = 0;
                _movement->y = y;
            ...
            else if(fabsf(xABS)>fabsf(yABS) && (fabsf(xABS)-fabsf(yABS))<0.5){
                float x = 2.5;
                if (xABS<0) x=-2.5;
                _movement->x = x;
                _movement->y = 0;
