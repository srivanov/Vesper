...
GraphNode * Graph::getInitialNode(dvector2D initialPosition){
    it = Grafo.begin();
    float MenorDistancia = -1.f, NuevaDistancia;
    int IDfinal;
    while (it!=Grafo.end()) {
        NuevaDistancia = EasyMath::CalcularDistancia(initialPosition,it->second->getPosition() );
...
GraphNode * Graph::getFinalNode(dvector2D finalPosition){
    it = Grafo.begin();
    float MenorDistancia = -1.f, NuevaDistancia;
    int IDfinal;
    while (it!=Grafo.end()) {
