    ...
    while(layer){
        ...
        for(int k=0; k < w ; k++){
            ...
            if(m_numLayers==0)
                ...
                int y = (int) k / (m_width-1) ;
                int x = (int) k % (m_width-1) ;
...
void loadLevel::CreateWorld(){
    ...
    Muro * muro = new Muro;
    muro->construirMuros(m_tilemap,objetos, pos_objetos, m_tileWidth, m_tileHeight);
    ...
    for (; it<end; it++) {
        ...
        {
            g = factory.PObject(it, pos, tipo);
            Mundo.push_back(g);
        }
        else if(tipo == ALARMA ||
                tipo == PUERTA ||
               (tipo > BALA && tipo < LLAVE))
        {
            g = factory.WObject(it, pos, tipo);
            Mundo.push_back(g);

            
        }
        else if(tipo==REHEN || tipo==ENEMIGOS){
            g = factory.Actores(it, pos, tipo);
            Mundo.push_back(g);
