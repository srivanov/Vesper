...
triggers::triggers(TypeEvents& type, unsigned int& idTrigger, unsigned int* idSource, vector3D& pos, float& radio, float& duration){
    _type = type;
...
unsigned long trigger_system::add_trigger(TypeEvents& type, unsigned int* id, vector3D& pos, float& radio, float& duration){
